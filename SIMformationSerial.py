...
def set_bench_vars(sim_data, bench_number, new_mph, new_time, status):
    ...
    sim_data.hilDataVec[bench_number].hilCurDistance += (
        (new_time - prev_time)*(prev_mph + new_mph)/2)
...
def find_bench(bench, abs_time, speed, status, sim_data, reset=False):
    ...
    if reset:
        ...
        for i in range(1, sim_widget.guiData.numHILs + 1):
            ...
            if bench == bench_str:
                sim_data = set_bench_vars(
                    sim_data, i - 1, current_mph, current_time, status)
